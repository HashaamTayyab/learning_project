...
from .forms import TrackForm, ModuleForm, SubDotForm, TopicForm, ContentForm
...
def view_modules(request, track_id):
    track = Track.objects.get(id=track_id)
    modules = Module.objects.filter(track=track)
    return render(request, 'editor/modules.html', {'track': track, 'modules': modules})

# View Subdots

def view_subdots(request, module_id):
    module = Module.objects.get(id=module_id)
    subdots = SubDot.objects.filter(module=module)
    return render(request, 'editor/subdots.html', {'module': module, 'subdots': subdots})
...
def content_addition(request):
    ...
    return render(request, 'editor/content_addition.html', {'modules': modules, 'subdots': subdots})
...
def add_subdot(request, module_id):
    module = Module.objects.get(id=module_id)
    if request.method == 'POST':
        name = request.POST.get('name')
        subdot = SubDot(module=module, name=name)
        subdot.save()
        messages.success(request, 'Subdot added successfully!')
        return redirect('view_subdots', module_id=module.id)
    return render(request, 'editor/add_subdot.html', {'module': module})
